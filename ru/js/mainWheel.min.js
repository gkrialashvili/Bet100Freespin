...
!(function () {
  var POINTS_LIMIT = 500,
    ...
    giftTypes = {
      ...
      freebet: { ge: "₾ ფრიბეთი", en: "₾ freebet", ru: "₾ фрибетов" },
      bonus: {
    ...
    slotSpinsByLevel_50 = 1,
    slotSpinsByLevel_250 = 2,
    slotSpinsByLevel_500 = 3,
    slotSpinsByLevel_2500 = 4,
    slotSpinsByLevel_5000 = 5,
    sportSpinsBylevel_5 = 1,
    sportSpinsBylevel_20 = 2,
    sportSpinsBylevel_50 = 3,
    sportSpinsBylevel_200 = 4,
    sportSpinsBylevel_500 = 5,
    sportSpinsBylevel_1000 = 6,
    wheelTypes_SLOT = "slots",
    wheelTypes_SPORT = "sport",
    spinsUsedUp_SLOT = 0,
    spinsUsedUp_SPORT = 0;
  ...
  function setBarsInitial() {
    var fullers;
    document.querySelectorAll(".progress_line").forEach(function (node) {
      node.innerHTML = "";
      var segments = [
        createSegment("0"),
        createSegment("25%"),
  ...
  function createSegment(leftOffset) {
    ...
    return (filler.style.height = "10px"), seg.appendChild(filler), seg;
  }
  function addListeners(userData) {
    var _dropOldEvents = dropOldEvents(getSpinBtns()),
      slotSpinner = _dropOldEvents.slotSpinner,
      sportSpinner = _dropOldEvents.sportSpinner,
      _getWheels = getWheels(),
      slotWheel = _getWheels.slotWheel,
      sportWheel = _getWheels.sportWheel;
    slotSpinner.addEventListener("click", function () {
      0 !== remainingSlotSpins && draw(userData, 5, slotWheel);
    }),
      sportSpinner.addEventListener("click", function () {
        0 !== remainingSportSpins && draw(userData, 4, sportWheel);
      });
  }
  function dropOldEvents(btns) {
    var slotSpinner = btns.slotSpinner,
      sportSpinner = btns.sportSpinner;
    return {
  ...
  function cloneNode(node) {
    var parent = node.parentNode,
      newBtn = node.cloneNode(!0);
    return node.remove(), parent.appendChild(newBtn), newBtn;
  }
  function getUserData(userData) {
    getSlotData(userData), getSportData(userData);
  }
  function getSlotData(userData) {
    fetch("https://cms.crocobet.com/campaigns/daily-slot-wheel-080221/user", {
      headers: { "X-ODDS-SESSION": userData.xOdds },
    })
      .then(function (r) {
        return r.json();
      })
      .then(function (res) {
        ...
        if (res && res.data && res.data.metadata) {
          var userInfo = res.data.metadata,
            pointAmount = userInfo.points % 5e3,
            level = userInfo.levels[userInfo.unlockedLevel - 1];
          (remainingSlotSpins =
            userInfo.unlockedLevel - userInfo.currentLevel) < 0 &&
            (remainingSlotSpins = 0),
          ...
          var basePoints = userInfo.levels[userInfo.unlockedLevel - 1] || 0,
            ...
            extraPoints,
            extraPercentage =
              ((pointAmount - basePoints) /
                Math.max(nextPoints - basePoints, 1)) *
              100;
          drawBar(
            ...
            0 !== remainingSlotSpins && slotSpinner.classList.add("active");
        }
      })
      .catch(function (err) {
      ...
      .then(function (res) {
        ...
        if (res && res.data && res.data.metadata) {
          ...
          drawBar(
            ...
            0 !== remainingSportSpins && sportSpinner.classList.add("active");
        }
      })
      .catch(function (err) {
  ...
  function drawBar(
    ...
    for (
      var line = document.querySelectorAll(".progress_line")[barNum],
        barColor = 0 === barNum ? "#4963d7" : "#03b730",
        segments = line.children,
        currentSegment = 0;
    ...
    if (0 !== currentSegment && segments[currentSegment]) {
    ...
    var cursor,
      ...
      cursorIncrementInSegment = (25 * percentageInSegment) / 100,
      zeroInSegment = 6 === currentSegment,
      cursorLbl;
    (document.querySelectorAll(".ind_wrp")[barNum].style.left =
      0 === currentSegment
        ? "0"
        : "calc(" +
          filledSegments +
          "% + " +
          (zeroInSegment ? 0 : cursorIncrementInSegment) +
          "%)"),
      (document.getElementById(
        0 === barNum ? "slot_prgs" : "sport_prgrs"
      ).innerHTML =
    ...
    if (
      (progressPts.forEach(function (pt) {
        pt.classList.remove("active"),
          pt.children[0].classList.remove(0 === barNum ? "active-b" : "active");
      }),
      0 !== currentSegment)
    )
  ...
  function draw(userData, wheelId, wheel) {
    ...
    fetch("https://cms.crocobet.com/campaigns/sport-wheel-1502/get-prize", {
      ...
      .then(function (r) {
        ...
        throw new Error("დაფიქსირდა შეცდომა სცადეთ მოგვიანებით");
      })
      .then(function (res) {
        var _getSpinBtns = getSpinBtns(),
          slotSpinner = _getSpinBtns.slotSpinner,
          sportSpinner = _getSpinBtns.sportSpinner,
          prize,
          prizeWon,
          prizeId,
          prizeType;
          ...
          : 4 === WHEEL_ID &&
            (function () {
              ...
              remainingSportSpins &&
                ...
                (getLabels().sportLbl.innerHTML = remainingSportSpins),
                0 === remainingSportSpins &&
                  sportSpinner.classList.remove("active"));
            })();
      })
      .catch(function (err) {
        var Error;
        console.error("ERROR: ", err), showErrorModal(errorMessage[langToken]);
      });
  }
  function spinTheWheel(wheelRef, id, wheelId) {
    var rndSpins,
      angleOnWheel,
      rotAmount =
        360 * Math.floor(3 + Math.random()) +
        (5 === wheelId ? slotPrizes.get(id) : sportPrizes.get(id));
    (wheelRef.style.transition =
      "all 5s cubic-bezier(0.25, 0.1, 0.25, 1) 0.1s"),
  ...
  function openGiftPopup(prizeAmount, prizeLabel, prizeBonus) {
    var giftModal = document.querySelector(".popup-frsp"),
      prizeLbl = document.getElementById("frsp"),
      gifCongrats = document.querySelector(".text-block-638");
    giftModal.classList.remove("hide");
    var langToken = getLangToken();
    if (
      ("5burninghotfs" === prizeLabel
        ...
        : "freebet" === prizeLabel &&
          ...
          (document.getElementById("popPlayMob").href =
            "https://www.crocobet.com/#/sports/prematch")),
      !0 === prizeBonus && 10 === prizeAmount)
    ) {
      var prize = "gel" === prizeLabel ? giftTypes : giftTypes.bonus[langToken];
      (gifCongrats.style.display = "none"),
        (document.querySelector(".text-block-639").style.marginTop = "110px");
    } else if (!0 === prizeBonus && 5 === prizeAmount) {
      var prize =
        "gel" === prizeLabel ? giftTypes : giftTypes.bonusFreebet[langToken];
      (gifCongrats.style.display = "none"),
        (document.querySelector(".text-block-639").style.marginTop = "110px");
    } else {
      var prize =
        "gel" === prizeLabel ? giftTypes : giftTypes[prizeLabel][langToken];
    ...
    (prizeLbl.innerHTML = prizeAmount + " " + prize), listenForPopupQuit();
  }
  function getLangToken() {
    var url,
      baseUrl,
      urlSegments = window.location.href.split("?")[0].split("/");
    return urlSegments[urlSegments.length - 2].split(".")[0];
  }
  function listenForPopupQuit() {
    var giftPopupCloseBtn;
    document
      .querySelector(".close_pop")
  ...
  function resetWheelSpin() {
    var _getWheels2 = getWheels(),
      slotWheel,
      sportWheel;
    [_getWheels2.slotWheel, _getWheels2.sportWheel].forEach(function (wheel) {
    ...
    fetch(
      ...
      .then(function (historyData) {
        ...
        historyData &&
          historyData.data &&
          updateHistoryList(
            historyData.data.map(function (historyItem) {
              return {
  ...
  function showErrorModal(error) {
    let errorText = document.querySelectorAll("#error-text"),
      errorModal = document.querySelector(".error-modal-container");
    (errorText.innerHTML = error), (errorModal.style.display = "flex");
  }
  function updateHistoryList(historyItems, wheelId) {
    var containers = document.querySelectorAll("#slotboardsr"),
      container = 5 === wheelId ? containers[0] : containers[1];
    (container.innerHTML = ""),
      historyItems.forEach(function (item) {
        var date = new Date(item.time),
          hItem = document.createElement("div");
        hItem.classList.add("borbal_lb_content"),
          4 === wheelId && hItem.classList.add("sport");
        var timeLbl = document.createElement("div"),
          prizeLbl = document.createElement("div");
        ...
        var langToken = getLangToken();
        !0 === item.prize.bonus && 10 === item.prize.amount
          ? (prizeLbl.innerHTML =
              item.prize.amount + " " + giftTypes.bonus[langToken])
          : !0 === item.prize.bonus && 5 === item.prize.amount
          ? (prizeLbl.innerHTML =
  ...
  function getSpinBtns() {
    var slot, sport;
    return {
      slotSpinner: document.getElementById("slotSpinner"),
      sportSpinner: document.getElementById("sportSpiner"),
    };
  }
  ...
  (window.onload = function () {
    ...
    document
      ...
      .addEventListener("click", function () {
        let errorModal;
        document.querySelector(".error-modal-container").style.display = "none";
      });
})();
var dt = new Date(),
